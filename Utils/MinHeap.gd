...
func insert(node) -> void:
	...
	while index != 0 and heap[parent_index(index)].cost > heap[index].cost:
...
func min_heapify(index: int) -> void:
	...
	if left < heap.size() and heap[left].cost < heap[smallest].cost:
		smallest = left
	
	if right < heap.size() and heap[right].cost< heap[smallest].cost:
		smallest = right
